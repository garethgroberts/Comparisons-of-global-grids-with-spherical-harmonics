...
### SEE run_example_fixed.sh ###

# $1 = max sph harm degree IN DENSITY FILE
# $2 = nr of layers, nr+1 = no of radial grid points
# $3 = max sph harm degree FOR DT CALCULATION
# $4 = min depth to use for DT? (and grav etc.) calcs
# $5 = kappa index i.e. is density model compressible? 1 if yes (Sia etc.), anything else (0) if no (Mc2 Model0 and 1, etc.)
# $6 = rho_w (air loaded or water loaded?)
# $7 = visc0 (reference viscosity for visc profs - also feeds into matrices?)
# $8 = l_min (minimum sph harm coeff for calculation - mostly 1? Can't include degree 0.)
# $9 = bgrho (reference density, NOT the same as density to be used for rho_m, which in updated code is worked out from rho0(nr+1). See Panasyuk Eq 11.
# $10 = grav10 (flag to set little g = 10.0 everywhere (using "grav" function in matrix.f)

echo "   integer,       parameter :: i4b = selected_int_kind(9)
   ...
   !> ------- SIZE PARAMETERS -----------
   !> maximum spherical harmonic degree in the denisty file 
   integer(i4b),  parameter :: l_max = $1
   !> minimum spherical harmonic degree for computation 
   integer(i4b),  parameter :: l_min = $8   
   !> maximum spherical harmonic degree for computation 
   integer(i4b),  parameter :: l_max_calc = $3
   !> number of radial layers (with tickness)
   !> with nr+1 number of radial grid points
   integer(i4b),  parameter :: nr = $2 
   ...
   integer(i4b),  parameter:: visual3d = 1 
   ...
   integer(i4b),  parameter:: kappaindex=$5
   !> Starting Depth for calculating the gravity based on
   real(qp),      parameter:: top_depth =  $4 
   real(qp),      parameter:: bot_depth = -10 
   !> Flag to use constant little g (grav_val) of 10.0, as TERRA incompressible models use
   integer(i4b),  parameter:: grav10 = ${10}
   !> boundary condition on top 
   integer(i4b),  parameter:: bdry=${11}
   !> boundary condition on bot 
   integer(i4b),  parameter:: bdry_bot=${12}
   !> background density
   real(qp),      parameter:: bgrho = $9
   !> background Viscosity
   real(qp),      parameter:: visc0 = $7    
