...
!      endif
     ...
     &        *(visc0/(bgrho*r(nr+1)*grav(r(nr+1))))*u(3)
            ...
            ! print *, bgrho, rho0(nr+1), rho0(1)
            SurKernel(j,i)       =
     &          -(visc0/((rho0(nr+1)-rho_w)*r(nr+1)*grav(r(nr+1)))) !NOTE THAT GGR INCLUDED THIS LINE
!     &     -(visc0/((bgrho-rho_w)*r(nr+1)*grav(r(nr+1))))  !NOTE THAT GGR COMMENTED THIS LINE OUT
     &         *(u(1)+(rho_w/bgrho)*u(3))
